...
import { Character, DialogueNode, GameVariable } from "@/entities/all";
import {
  GitBranch,
  MessageSquare,
  Plus,
...
export default function NodeEditor({ 
  ...
  const startConnection = useCallback((nodeId, position, outputIndex = 0) => {
    ...
    if (connectionStart && connectionStart.nodeId !== targetNodeId) {
      ...
      if (sourceNode) {
        ...
        if (!connections.includes(targetNodeId)) {
          updateNode(connectionStart.nodeId, {
            connections: [...connections, targetNodeId]
          });
        }
      }
    }
  ...
  const generateAIDialogue = async (selectedNodeData) => {
    if (!selectedNodeData.character_id) {
...
Generate 4 different dialogue options that this character might say. Each should be 1-2 sentences and match their personality and voice style. Return them as a simple list.`;
  ...
  const nodeTypes = [
    { type: 'dialogue', icon: MessageSquare, label: 'Dialogue', color: 'blue' },
    { type: 'choice', icon: GitBranch, label: 'Choice', color: 'purple' },
    { type: 'condition', icon: Settings, label: 'Condition', color: 'yellow' },
    { type: 'action', icon: Settings, label: 'Action', color: 'orange' }
  ];
            ...
            <>
              <div>
                ...
                <Select 
                  ...
                  onValueChange={(value) => setEditData(prev => ({ ...prev, character_id: value }))}
                >
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Button
                    size="sm"
                    onClick={() => generateAIDialogue(editData)}
          ...
          <svg className="absolute inset-0 pointer-events-none" style={{ zIndex: 5 }}>
            ...
            {nodes.map(node => 
              node.connections?.map((targetId, connIndex) => {
                ...
                const sourceX = (node.position?.x || 0) + 256;
                const sourceY = (node.position?.y || 0) + 60;
                ...
                return (
                  <path
                    key={`${node.node_id}-${targetId}-${connIndex}`}
                    d={path}
